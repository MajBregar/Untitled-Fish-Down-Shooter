...
public class StandardEnemyController : MonoBehaviour
{
    ...
    public float bubbleProjectileSpeed = 4f;
    ...
    public Vector3 bubbleSpawnPosition; // Position from which the bubble is fired
        ...
        switch (curState){
            ...
            case EnemyState.MeleeAttacking:
                if (Time.time >= meleeDurationTimer) {
                    ...
                    if (hit == false){
                        rangedWaitTimer = Time.time + RangedWaitDuration;
                        curState = EnemyState.WaitForRanged;
                        StopMovement();
            ...
            case EnemyState.WaitForRanged:

                if (distanceToPlayer < RangedAttackRange && Time.time >= rangedWaitTimer) {
                    curState = EnemyState.ChasingPlayer;
                } else if (distanceToPlayer >= RangedAttackRange) {
                    rangedAttackTimer = Time.time + RangedAttackDuration;
                    curState = EnemyState.RangedAttack;
                    enemyAnimator.SetTrigger("Shoot");
                }

                break;
            case EnemyState.RangedAttack:
                if (Time.time >= rangedAttackTimer){
    ...
    private void PerformRangedAttack()
    {
        ...
        if (bubbleRb != null)
        {
            bubbleRb.linearVelocity = direction * bubbleProjectileSpeed;
        }
